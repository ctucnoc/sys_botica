...
import com.sys.botica.crce.pe.sys_botica.errorhandler.SysBoticaGenericServerException;

@Component
public final class SysBoticaUtil {
	...
	public Pageable getPageable(Integer page, Integer size, Optional<Integer> order, Optional<String> field) {
		Optional<Integer> sortOrder = order;
		Optional<String> sortField = field;
	...
	public static HrefEntityDTO createHrefFromResource(Object id, SysBoticaResource resource)
			throws SysBoticaGenericServerException {
		HrefEntityDTO hrefEntity = new HrefEntityDTO();
		try {
			StringBuilder builder = new StringBuilder();
			Field field = SysBoticaConstant.class.getDeclaredField("RESOURCE_" + resource + "S");
			Object valueResource = field.get("");
			builder.append(valueResource);
			field = SysBoticaConstant.class.getDeclaredField("RESOURCE_" + resource + "S_" + resource);
			valueResource = field.get("");
			builder.append(valueResource).append("/").append(id);
			hrefEntity.setId(id);
			hrefEntity.setHref(builder.toString());
		} catch (Exception e) {
			throw new SysBoticaGenericServerException("Error generating href resource");
		}
		return hrefEntity;
	}
